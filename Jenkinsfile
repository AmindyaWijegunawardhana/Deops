        ...
        stage('Health Checks') {
            steps {
                script {
                    ...
                    for i in {1..30}; do
                        if docker exec happy-tails-backend node -e "require('http').get('http://51.20.78.235:5000/api/health', (r) => {if (r.statusCode !== 200) throw new Error(r.statusCode)})" 2>/dev/null; then
                    ...
                    for i in {1..30}; do
                        if docker exec happy-tails-frontend wget --quiet --tries=1 --spider http://51.20.78.235/index.html 2>/dev/null; then
    ...
    post {
        success {
            script {
                ...
                - Frontend: http://51.20.78.235:80
                - Backend API: http://51.20.78.235:5000/api
                =================================================
                '''
            }
        }
