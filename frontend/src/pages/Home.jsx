...
function Home() {
  ...
  const { isPetAdopted } = useAdoption()

  // Filter pets based on type, adoption status, and search query
  const filteredPets = pets.filter(pet => {
    const isAdopted = isPetAdopted(pet.id)
    const matchesType = filterType === 'All' || pet.type === filterType
    const matchesAdoption = adoptionFilter === 'Show All' ||
      (adoptionFilter === 'Available' && !isAdopted) ||
      ...
      <section className="bg-gradient-to-r from-primary to-primary-700 text-white py-6">
        <div className="max-w-container mx-auto px-container text-center">
          <h1 className="text-2xl font-heading font-bold mb-2">
            Adopt a Street Pet- give them a happy smile
          </h1>
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className={`px-6 py-2 rounded-full font-medium transition-all duration-200 ${filterType === 'All'
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full font-medium transition-all duration-200 flex items-center gap-2 ${filterType === 'Dog'
                      ? 'bg-primary text-white shadow-md'
                      : 'bg-neutral-100 text-text-secondary hover:bg-neutral-200'}`}
                >
                  🐕 Dogs
                ...
                <button
                  ...
                  className={`px-6 py-2 rounded-full font-medium transition-all duration-200 flex items-center gap-2 ${filterType === 'Cat'
                      ? 'bg-primary text-white shadow-md'
                      : 'bg-neutral-100 text-text-secondary hover:bg-neutral-200'}`}
                >
                  🐈 Cats
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${adoptionFilter === 'Show All'
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${adoptionFilter === 'Available'
                      ? 'bg-accent text-white shadow-md'
                      : 'bg-neutral-100 text-text-secondary hover:bg-neutral-200'}`}
                >
                  Available
                ...
                <button
                  ...
                  className={`px-4 py-1.5 rounded-full text-sm font-medium transition-all duration-200 ${adoptionFilter === 'Adopted'
                      ? 'bg-accent text-white shadow-md'
                      : 'bg-neutral-100 text-text-secondary hover:bg-neutral-200'}`}
                >
                  Adopted
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden rounded-lg mb-4">
                      <img
                        src={pet.image}
                        alt={pet.name}
                        className={`w-full object-cover transition-transform duration-300 group-hover:scale-110 ${isAdopted ? 'blur-sm grayscale-[30%]' : ''}`}
                        style={{
                          height: '220px',
                          filter: isAdopted ? 'blur(2.5px) grayscale(30%)' : 'none'
                        }} />
